  ...
  if (channel_songs[1] > -1)
      ...
      else
	{
	  if (channel_songs[0] < 0)
	    {
	      mprintf(sizeof(buf),buf,"Music: %s, %s",
	    ...
	    {
	      mprintf(sizeof(buf),buf,"Music: '%s'",
	  ...
	  for (d = descriptor_list; d != NULL; d = d->next)
	    {
	      ...
	      if ( d->connected == CON_PLAYING &&
		   !IS_SET(victim->chan_flags,CHANNEL_NOMUSIC) &&
  ...
  for (obj = object_list; obj != NULL; obj = obj->next)
    {
      if (obj->next == obj)
	{
	  bugf("song_update.  obj loop detected. obj %d  %s\n\r", 
	       obj->pIndexData->vnum, interp_cmd);
	  obj->next = NULL;
	  crash_fix();
	  exit(1);
	}
      ...
      if ((room = obj->in_room) == NULL)
	{
      ...
      if (obj->value[0] < 0)
	{
	  mprintf(sizeof(buf),buf,"$p starts playing %s, %s.",
      ...
      else
	{
      ...
      mprintf(sizeof(buf),buf,"$p: '%s'",line);
      if (room->people != NULL)
	act(buf,room->people,obj,NULL,TO_ALL);
    }
}
...
void do_play(CHAR_DATA *ch, char *argument)
{
  ...
  if (!str_cmp(arg,"list"))
	...
	match = TRUE;
      ...
      mprintf(sizeof(buf),buf,"%s has the following songs available:\n\r",
	      juke->short_descr);
