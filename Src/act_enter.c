...
ROOM_INDEX_DATA  *get_random_room(CHAR_DATA *ch)
{
  ...
  for ( ; ; )
  {
    ...
    if ( room )
      ...
      && !IS_SET( room->room_flags, ROOM_SHIP )
...
//    ||    IS_SET( old_room->room_flags, ROOM_NO_RECALL ) ) )
    ...
    for ( fch = old_room->people; fch; fch = fch_next )
    {
      ...
      if ( fch->master == ch && fch->position == POS_STANDING)
      {
        ...
        if ( IS_NPC(fch) )
        {
          if ( fch->pIndexData->vnum != MOB_VNUM_QUEST )
          {
          ...
          else
          {
            if ( portal
            && ( portal->value[0] != -1 )
            && ( portal->pIndexData->vnum != OBJ_VNUM_PORTAL ) )
              do_function( fch, &do_enter, argument ); // rescue quest mob, and not a player portal
          }
        }
        else
        {
