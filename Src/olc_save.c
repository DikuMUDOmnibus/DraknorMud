...
#define DIF(a,b) (~((~a)|(b)))
#define CURRENT_AFILE_VERSION  1  /* 0 - mobiles: aff2 */
...
void save_area_list()
{
  ...
  if ( ( fp = fopen( "area.lst", "w" ) ) == NULL )
  ...
  else
    {
       ...
       */
#if TEST_MUD
      fprintf( fp, "/master/area/help.are\n");
#else
      fprintf( fp, "help.are\n");
#endif
...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
  ...
  fprintf( fp, "%d ",       pMobIndex->frag_number );
  ...
  fprintf( fp, "L %s~\n", fix_string( pMobIndex->long2_descr ) );
  fprintf( fp, "P %s~\n", fix_string( pMobIndex->path ) );
// this seems to be hosing the .are files.  Thankfully I had the foresight to back them all up!
  fprintf( fp, "R %d\n", pMobIndex->max_riders );
...
void save_object( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
  ...
  switch ( pObjIndex->item_type )
    ...
    case ITEM_LOCKER:
      if (pObjIndex->value[0])
  fprintf( fp, "%s %d %d %d %d\n",
     clan_table[pObjIndex->value[0]].name,
     pObjIndex->value[1], 
     pObjIndex->value[2],
     pObjIndex->value[3],
     pObjIndex->value[4]);
  ...
  for ( prog = pObjIndex->mprogs; prog; prog = prog->next )
    fprintf( fp, "M %s %d %s~\n", mprog_type_to_name( prog->trig_type ),
             prog->vnum, prog->trig_phrase );

  if ( pObjIndex->clan > 0 )
    fprintf ( fp, "C %s~\n" , clan_table[pObjIndex->clan].name );

  if ( pObjIndex->clan_name )
    fprintf ( fp, "L %s~\n" , pObjIndex->clan_name );
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
  ...
  {
  /* added by Merak 2006-09-20 to prevent ships messing with doors */
    for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; 
         pRoomIndex = pRoomIndex->next )
    {
      if ( IS_SET( pRoomIndex->room_flags, ROOM_SHIP )
      && ( pRoomIndex->area == pArea 
      || ( pRoomIndex->state >= 0 
      && pRoomIndex->exit[pRoomIndex->state]->u1.to_room->area == pArea ) ) )
        set_state_room( pRoomIndex, -1 );
    }
    /* end add - Merak */
  }
  ...
  for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
  {
    ...
    if ( pRoomIndex->area == pArea )
        ...
        fprintf( fp, "0 " );
        fprintf( fp, "%llu ",       pRoomIndex->room_flags );
...
/*
      ...
      locks = 4;
              ...
              if ( IS_SET( pExit->rs_flags, EX_MULTI ) )
                locks += 10;
              if ( IS_SET( pExit->rs_flags, EX_SEETHROUGH ) )
                locks += 20;
              if ( IS_SET( pExit->rs_flags, EX_NOEXIT ) )
                locks += 40;
              if ( IS_SET( pExit->rs_flags, EX_FENCED ) )
                locks += 80;
              if ( IS_SET( pExit->rs_flags, EX_OBSCURED ) )
                locks += 160;
              if ( IS_SET( pExit->rs_flags, EX_OBSCURED ) )
                locks += 320;
...
*/
          ...
          int newFlags = 0;
          if ( IS_SET( pExit->rs_flags, EX_ISDOOR ) )
              SET_BIT(newFlags, NEW_EX_ISDOOR);
          if ( IS_SET( pExit->rs_flags, EX_
                  */
          fprintf( fp, "D%d\n",      pExit->orig_door );
        ...
        fprintf ( fp, "C %s~\n" , clan_table[pRoomIndex->clan].name );
          ...
          if ( pRoomIndex->clan_name )
            fprintf ( fp, "L %s~\n" , pRoomIndex->clan_name );
                
        if (!IS_NULLSTR(pRoomIndex->owner))
        fprintf ( fp, "O %s~\n" , pRoomIndex->owner );

          for (prog = pRoomIndex->mprogs; prog; prog = prog->next)
            fprintf( fp, "P %s %d %s~\n",
                     mprog_type_to_name( prog->trig_type ),
                     prog->vnum, prog->trig_phrase );

          fprintf ( fp, "F"); // Custum fish VNUMs
          for (door = 0; door <= 7; door++) // might as well reuse a var instead of making a new one
            fprintf ( fp, " %d" , pRoomIndex->fish[door] );
          fprintf ( fp, "\n");
    ...
    if ( pRoomIndex->area == pArea )
      ...
      if ( ( pExit = pRoomIndex->exit[door] ) 
        ...
        fprintf( fp, "D 0 %d %d %d The %s door of %s is %s\n", 
           pRoomIndex->vnum,
           pExit->orig_door,
           IS_SET( pExit->rs_flags, EX_LOCKED) ? (IS_SET(pExit->rs_flags, EX_HIDDEN)? 3 : 2) : 1,
           dir_name[ pExit->orig_door ],
           pRoomIndex->name,
           IS_SET( pExit->rs_flags, EX_LOCKED) ? (IS_SET(pExit->rs_flags, EX_HIDDEN)? "closed, locked, and hidden" : "closed and locked")
           : "closed" );
#endif
#if !defined( VERBOSE )
        ...
        for ( pReset = pRoom->reset_first; pReset; pReset = pReset->next )
    {
      switch ( pReset->command )
        ...
        case 'G':
          ...
          if ( !pLastMob )
      {
        mprintf(sizeof(buf),buf, "Save_resets: !NO_MOB! in [%s]", pArea->file_name );
        bug( buf, 0 );
      }
          break;
...
void save_area( AREA_DATA *pArea )
{
  ...
  /*  mprintf(sizeof(buf),buf,"<[%-3d %-3d] %15s %15s>",pArea->low_range,
      pArea->high_range, fix_string(pArea->builders), pArea->name);*/
  mprintf(sizeof(buf),buf,"{%d %d} %-15s %-15s",pArea->low_range,
      pArea->high_range, fix_string(pArea->builders), pArea->name);
    
  fprintf( fp, "Range %d %d\n",    pArea->low_range, pArea->high_range );
  fprintf( fp, "Credits %s~\n",    pArea->credits );
  fprintf( fp, "Vers %d\n",       pArea->version );
  fprintf( fp, "Flags %s\n",      print_flags(pArea->flags));
  fprintf( fp, "Security %d\n",   pArea->security );
  fprintf( fp, "Align %d\n",      pArea->align );
  fprintf( fp, "Continent %d\n",  pArea->continent);
  fprintf( fp, "Reset_rate %d\n", pArea->reset_rate );
  fprintf( fp, "End\n\n\n\n" );
  ...
  //pArea->version = CURRENT_AFILE_VERSION;  
...
void do_asave( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !OLC_SECURITY_FLAGS( ch, OLC_SEC_SAVE ) )
  {
  send_to_char( "Insufficient security to save areas.\n\r", ch );
  return;
  }
  ...
  if ( !str_cmp( "changed", arg1 ) )
  {
      ...
      for( pArea = area_first; pArea; pArea = pArea->next )
    {
      ...
      {
        save_area( pArea );
        mprintf(sizeof(buf),buf, "%24s - '%s'", pArea->name, pArea->file_name );
     ...
     if ( ch )
        ...
        log_string( "Saved clans:" );

     mprintf( sizeof( buf1 ), buf1, "None.\n\r" );
     ...
     for ( clan = clan_free ; clan ; clan = clan->next )
     {
        ...
        {
            save_clan( clan );
            mprintf( sizeof( buf1 ), buf1, "%24s", clan->name );
...
#if TEST_MUD
  if ( !(fp = fopen( "/master/area/help.are", "w") ) )
#else
