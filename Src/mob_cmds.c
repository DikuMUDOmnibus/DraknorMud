...
const  struct  mob_cmd_type  mob_cmd_table  [] =
{
  ...
  { "atransfer",  do_mpatransfer },
  { "gatransfer", do_mpgatransfer},
  ...
  { "rstate",     do_mprstate    },
  { "setpath",    do_mpsetpath   },
  { "setquest",   do_mpsetquest  },
  { "peace",      do_mppeace     },
  { "setowner",   do_mpsetowner  },
  { "ostrname",   do_mpostrname  },
  { "ostrshort",  do_mpostrshort },
  { "ostrlong",   do_mpostrlong  },
  { "",           0              }
};
...
void mob_interpret( CHAR_DATA *ch, char *argument )
{
   ...
   */
  for ( cmd = 0; mob_cmd_table[cmd].name[0]; cmd++ )
  {
  ...
  mprintf(sizeof(buf),buf, "Mob_interpret: invalid cmd from mob %d: '%s'",
...
void do_mpstat( CHAR_DATA *ch, char *argument )
{
  ...
  mprintf( sizeof(arg), arg, "Mobile #%-6d [%s]\n\r",
           victim->pIndexData->vnum, victim->short_descr );
  send_to_char( arg, ch );

  mprintf( sizeof(arg),arg, "Delay   %-6d [%s]\n\r",
           victim->mprog_delay,
...
void do_mpgecho( CHAR_DATA *ch, char *argument )
{
  ...
  for ( d = descriptor_list; d; d = d->next )
  {
    if ( d->connected == CON_PLAYING )
    {
      ...
      printf_to_char(d->character,"%s{x\n\r",argument);
    }
  }
}
...
void do_mpassist( CHAR_DATA *ch, char *argument )
{
  ...
  if ( victim == ch || ch->fighting || victim->fighting == NULL )
    return;
...
void do_mpmload( CHAR_DATA *ch, char *argument )
{
  ...
  {
    mprintf( sizeof(arg), arg, "Mpmload: bad mob index (%d) from mob %d",
...
void do_mpat( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg[0] == '\0' || argument[0] == '\0' )
  {
    bugf( "Mpat - Bad argument from vnum %d. arg = %s",
          IS_NPC(ch) ? ch->pIndexData->vnum : 0, argument );
    return;
  }

  if ( ( location = find_location( ch, arg ) ) == NULL )
  {
    bugf( "Mpat - No such location from vnum %d, argument = %s",
          IS_NPC(ch) ? ch->pIndexData->vnum : 0, argument );
    return;
  }
...
void do_mptransfer( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !str_cmp( arg1, "all" ) )
  {
    ...
    for ( victim = ch->in_room->people; victim; victim = victim_next )
    {
      ...
      if ( !IS_NPC(victim) )
      {
        mprintf(sizeof(buf),buf, "%s %s", victim->name, arg2 );
 ...
 * Syntax: mob transfer [target|'all'] [location]
 */
void do_mpatransfer( CHAR_DATA *ch, char *argument )
{
  char             arg1[ MAX_INPUT_LENGTH ];
  char             arg2[ MAX_INPUT_LENGTH ];
  char       buf[MAX_STRING_LENGTH];
  ...
  if ( arg1[0] == '\0' )
  {
    bug( "Mpatransfer - Bad syntax from vnum %d.",
         IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
    return;
  }
  ...
  else
  {
    if ( ( location = find_location( ch, arg2 ) ) == NULL )
    {
      bug( "Mpatransfer - No such location from vnum %d.",
           IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
      return;
    }
 ...
 * Syntax: mob gtransfer [victim] [location]
 */
void do_mpgatransfer( CHAR_DATA *ch, char *argument )
{
  char             arg1[ MAX_INPUT_LENGTH ];
  char             arg2[ MAX_INPUT_LENGTH ];
  char       buf[MAX_STRING_LENGTH];
  ...
  if ( arg1[0] == '\0' )
  {
    bug( "Mpgatransfer - Bad syntax from vnum %d.",
         IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
    return;
  }
  ...
  for ( victim = ch->in_room->people; victim; victim = victim_next )
  {
    victim_next = victim->next_in_room;
    if ( is_same_group( who,victim ) ||
         (ch->pet && is_same_group(who, victim->master)) )
    {
      mprintf(sizeof(buf),buf, "'%s' %s", victim->name, arg2 );
      do_mpatransfer( ch, buf );
    }
  }
  return;
}
...
/*
 * Sets the state of a ship-room.
 * -1 means all exits closed, 0-3 choose an exit.
 * errorchecking is made in set_state_room...
 *
 * Syntax mob rstate # ( -1 to 3 )
 */
void do_mprstate( CHAR_DATA *ch, char *argument )
{
  /* added by Merak 17/9 -06 */
  char arg[ MAX_INPUT_LENGTH ];

  one_argument( argument, arg );
  if ( !is_number( arg ) )
  {
    bug("Mob rstate: invalid arg from vnum %d.",
        IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
    return;
  }
  set_state_room( ch->in_room,atoi( arg ) );
}
/*
 * Forces a mob to return to it's reset_room.
 *
 * Syntax mob recall
 */
void do_mprecall( CHAR_DATA *ch, char *argument )
{
  /* added by Merak 13/11 -06 */
  if ( ch->reset_room && ch->reset_room != ch->in_room )
  {
    act( "$n slowly fades away.", ch, NULL, NULL, TO_ROOM );
    move_to_room( ch, ch->reset_room );
    act( "$n slowly fades into view.", ch, NULL, NULL, TO_ROOM );
  }
}
...
void do_mpflee( CHAR_DATA *ch, char *argument )
{
  ...
  {
    // Check for both ship and EX_NOEXIT needed -- Merak
    door = number_door( );
...
/*
 * Lets the mobile strip an object or all objects from the victim.
...
/*
 *  mprestore allows a mob to restore itself,
 *  a character or mob in the room, or the room
 *  as a whole.
 *
 *  Syntax: mob restore
 *      mob restore <victim>
...
/*
 * Lets the mob open a door with no echo and without the need
 * to first unlock it.
 *
 * Syntax: mob open <direction>
 */
void do_mpopen( CHAR_DATA *ch, char *argument )
{
...
/*
 * Lets the mob close a door with no echo.
 * Lockstate is restored from rs_flag.
 *
 * Syntax: mob close <direction>
 */
void do_mpclose( CHAR_DATA *ch, char *argument )
{
  ROOM_INDEX_DATA *to_room;
  EXIT_DATA *pexit, *pexit_rev;
  int door;
  ...
  if ( ( to_room = pexit->u1.to_room )
       ...
       &&   pexit_rev->u1.to_room == ch->in_room )
  {
    if ( IS_SET( pexit_rev->rs_flags, EX_LOCKED ) )
...
/*
 * Lets the mob set its own path.
 *
 * Syntax: mob setpath <path-string>
 */
void do_mpsetpath( CHAR_DATA *ch, char *argument )
{
  free_string( ch->path );
  ch->path_ptr = NULL;
  ch->path = str_dup( argument, ch->path );
}
...
/*
 * Sets whether or not a quest bit is on
 *
 * Syntax: mob setquest [victim] [mpquest#] [0/1]
 */
void do_mpsetquest( CHAR_DATA *ch, char *argument )
{
  ...
  /* set the bit */
  if (onoff == 1)
    SET_BIT(victim->pcdata->mpquests, pow(2,qnum));
  else
    REMOVE_BIT(victim->pcdata->mpquests, pow(2,qnum));
 ...
 *
 * Syntax: mob setowner [obj] [name]
 */
void do_mpsetowner( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ( ( obj = get_obj_carry( ch, arg, ch ) ) )
  {
    ...
    if (strcmp(arg2,"none"))
      obj->owner = str_dup(capitalize(arg2), obj->owner);
  }
}

/*
 * Sets an object's name/keywords
 *
 * Syntax: mob ostrname [obj] [keywords]
 */
void do_mpostrname( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ( ( obj = get_obj_carry( ch, arg, ch ) ) )
  {
    ...
    obj->name = str_dup( arg2 , obj->name);
  }
}

/*
 * Sets an object's short desc
 *
 * Syntax: mob ostrshort [obj] [short_desc]
 */
void do_mpostrshort( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ( ( obj = get_obj_carry( ch, arg, ch ) ) )
  {
    ...
    strcat(arg2,"{x");
    obj->short_descr = str_dup( arg2, obj->short_descr );
  }
}

/*
 * Sets an object's long desc
 *
 * Syntax: mob ostrlong [obj] [long_desc]
 */
void do_mpostrlong( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ( ( obj = get_obj_carry( ch, arg, ch ) ) )
  {
    ...
    strcat(arg2,"{x");
    obj->description = str_dup( arg2, obj->description );
  }
}
