...
bool compressStart(DESCRIPTOR_DATA *desc, unsigned char telopt)
{
  ...
  logit("Compression starting is verion 2: %s", (telopt ==
						 TELOPT_COMPRESS)
	? "NO": "YES");
  ...
  if (telopt == TELOPT_COMPRESS)
    ...
    bugf("compressStart: bad TELOPT passed");

  
  /* now we're compressing */
  desc->compressing = telopt;
  desc->out_compress = s;
  return TRUE;
}
...
void do_compress( CHAR_DATA *ch, char *argument )
{
  ...
  if (!ch->desc->out_compress) {
    ...
    write_to_buffer( ch->desc, compress2_do, 0 );
    write_to_buffer( ch->desc, compress_do, 0 );
