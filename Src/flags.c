...
void do_flag(CHAR_DATA *ch, char *argument)
{
  ...
  CHAR_DATA *victim;
  int64 *flag, old = 0, newf = 0, marked = 0, pos;
  char type;
  ...
  if (!str_prefix(arg1,"mob") || !str_prefix(arg1,"char"))
      ...
      else if (!str_prefix(arg3,"comm"))
	{
      ...
      else if (!str_prefix(arg3,"penalty"))
	{
	  if (IS_NPC(victim))
	    {
	      send_to_char("Penalty can't be set on NPCs.\n\r",ch);
	      return;
	    }

	  flag = &victim->pen_flags;
	  flag_table = pen_flags;
	}
      else if (!str_prefix(arg3,"chan"))
	{
	  if (IS_NPC(victim))
	    {
	      send_to_char("Channels can't be set on NPCs.\n\r",ch);
	      return;
	    }

	  flag = &victim->chan_flags;
	  flag_table = chan_flags;
	}
      ...
      for (pos = 0; flag_table[pos].name != NULL; pos++)
	{
	  if (!flag_table[pos].settable && IS_SET(old,flag_table[pos].bit))
	    {
	      SET_BIT(newf,flag_table[pos].bit);
	      continue;
	    }
	  ...
	  if (IS_SET(marked,flag_table[pos].bit))
		...
		case '+':
		  ...
		  break;
		case '-':
		  REMOVE_BIT(newf,flag_table[pos].bit);
		  break;
		default:
		  if (IS_SET(newf,flag_table[pos].bit))
		    REMOVE_BIT(newf,flag_table[pos].bit);
		  else
		    SET_BIT(newf,flag_table[pos].bit);
		}
...
#if AR
/*
 * Return the no_match string for a flag table.
 * Local function only.
 */
char *flag_no_match( const struct flag_type *flag_table )
{
  int flag;
  ...
  for ( flag = 0; flag_stat_table[flag].structure; flag++ )
	...
	return flag_stat_table[flag].no_match;
    }
  return "none";
}

/*
 * Returs a string with name(s) of the flags or stat entered, ignoring
 * visibility boolean.  Used in save routines.
 * Can be called up to 4 times within a single statement.
 */
char *flag_string_save( const struct flag_type *flag_table, int bits )
{
  ...
  return ( buf[cnt][0] != '\0' ) ? buf[cnt] + 1 : flag_no_match( flag_table );
}
