...
extern int64 flag_lookup args((const char *name, const struct flag_type *flag_table));
...
void load_mobiles( FILE *fp )
{
  ...
  for ( ; ; )
  {
      ...
      if ( letter != '#' )
        {
    bug( "(db2.c) Load_mobiles: # not found.", 0 );
    exit( 1 );
        }
      ...
      exit( 1 ); // Serious case, exit prog..
      }
      fBootDb = TRUE;
 
      pMobIndex                       = alloc_perm( sizeof( *pMobIndex ) );
      pMobIndex->path                 = &str_empty[0];
      pMobIndex->max_riders           = 0;
      pMobIndex->vnum                 = vnum;
      ...
      if ( is_number( tmpstr ) )
      {
      pMobIndex->max_count        = atoi( tmpstr );
      pMobIndex->short_descr        = fread_string( fp );
      }
      else
      {
      pMobIndex->max_count        = 500;
      pMobIndex->short_descr        = str_dup( tmpstr, 
                                        pMobIndex->short_descr );
      }
      if ( pMobIndex->max_count == 0 )
      pMobIndex->max_count        = 500;
      free_string( tmpstr );
      ...
      {
      tmpstr                        = fread_string( fp );
      pMobIndex->race             = race_lookup( tmpstr );
      free_string( tmpstr );
      }
      ...
      pMobIndex->frag_number          = fread_number( fp );
        ...
        if ( !( trigger = flag_lookup( word, mprog_flags ) ) )
      {
        ...
        pMobIndex->mprogs   = pMprog;
          ...
          {
        pMprogList = pMobIndex->mprogs;
        while ( pMprogList->next )
          pMprogList = pMprogList->next;
        pMprogList->next  = pMprog;
      }
      }
        else if ( letter == 'L' )
        {
          pMobIndex->long2_descr        = fread_string( fp );
          pMobIndex->long2_descr[0]     = UPPER( pMobIndex->long2_descr[0] );
        }
        else if ( letter == 'P' )
        {
        ...
        {
          pMobIndex->max_riders         = fread_number( fp );
        }
      else
      {
        ungetc( letter, fp );
...
void load_objects( FILE *fp )
{
  ...
  case ITEM_FOUNTAIN:
    ...
    pObjIndex->value[2]         = liq_lookup(fread_word(fp));
      if ( pObjIndex->value[2] < 0 ) pObjIndex->value[2] = 0;
    pObjIndex->value[3]         = fread_number(fp);
    pObjIndex->value[4]         = fread_number(fp);
    break;
  case ITEM_WAND:
  ...
  case ITEM_SCRY_MIRROR:
    ...
    break;
  case ITEM_LOCKER:
    pObjIndex->value[0]    = clan_lookup(fread_word(fp));
  ...
  default:      pObjIndex->condition = 100; break;
    ...
    bugf( "M[%5d] extra_descr doesn't end with \\n\\r", vnum );
      ...
      else if ( letter == 'M' )
      {
        ...
        prog->next          = NULL;
        if ( pObjIndex->mprogs == NULL )
          pObjIndex->mprogs = prog;
        else
        {
          proglist =pObjIndex->mprogs;
     ...
     else if ( letter == 'C') /* clan */
      {
        ...
        pObjIndex->clan      = clan_lookup(fread_string(fp));
          free_string( pObjIndex->clan_name );
          pObjIndex->clan_name = str_dup( clan_table[ pObjIndex->clan ].name, pObjIndex->clan_name );
      }
        ...
        {
            free_string( pObjIndex->clan_name );
            pObjIndex->clan_name   = fread_string( fp );
        }
...
void convert_mobile( MOB_INDEX_DATA *pMobIndex )
{
  ...
  (hmm.. must be some roundoff error in my calculations.. smurfette got
   ...
   approximately right..)
    ...
    pMobIndex->material          = str_dup("none", pMobIndex->material);
